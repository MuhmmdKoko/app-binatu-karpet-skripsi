...
$id = intval($_GET['id'] ?? 0);
$res = mysqli_query($konek, "SELECT * FROM promosi WHERE id_promosi=$id");
$data = mysqli_fetch_assoc($res);
if (!$data) {
    echo '<script>alert("Data tidak ditemukan.");window.location="?page=promosi_read";</script>';
    exit;
}
...
if (isset($_POST['simpan'])) {
    ...
    if (empty($judul) || empty($nilai_promo)) {
        ...
        $today = date('Y-m-d');
        $tanggal_berakhir_clean = str_replace("'", "", $tanggal_berakhir);
        if (!empty($tanggal_berakhir_clean) && $tanggal_berakhir_clean < $today && $status_promo == 'aktif') {
        ...
        $sql = "UPDATE promosi SET 
                    ...
                    syarat_min_transaksi = $syarat_min_transaksi, 
        ...
        if (mysqli_query($konek, $sql)) {
            echo '<script>alert("Promosi berhasil diperbarui.");window.location="?page=promosi_read";</script>';
            exit;
        } else {
            $err = 'Gagal memperbarui promosi: ' . mysqli_error($konek);
        }
    }
    // Jika ada error, data form akan diisi dari POST, bukan dari DB lagi
    $data = $_POST;
}
?>
<div class="container-fluid mt-4">
    <h3 class="mb-3">Edit Promosi</h3>
    ...
    <form method="post" class="card">
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="kode_promo" name="kode_promo" value="<?= htmlspecialchars($data['kode_promo']) ?>" placeholder="e.g., DISKON20">
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="tipe_promo" name="tipe_promo">
                            <option value="persen" <?= ($data['tipe_promo'] ?? '') == 'persen' ? 'selected' : '' ?>>Persen (%)</option>
                            <option value="nominal" <?= ($data['tipe_promo'] ?? '') == 'nominal' ? 'selected' : '' ?>>Nominal (Rp)</option>
                        </select>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="number" step="any" class="form-control" id="nilai_promo" name="nilai_promo" value="<?= htmlspecialchars($data['nilai_promo']) ?>" required>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="mb-3">
                        <label for="syarat_min_transaksi" class="form-label">Min. Transaksi (Rp)</label>
                        <input type="number" class="form-control" id="syarat_min_transaksi" name="syarat_min_transaksi" value="<?= htmlspecialchars($data['syarat_min_transaksi']) ?>" placeholder="Kosongkan jika tidak ada">
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" id="tanggal_mulai" name="tanggal_mulai" value="<?= htmlspecialchars($data['tanggal_mulai'] ?? '') ?>">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="mb-3">
                        <label for="tanggal_berakhir" class="form-label">Tanggal Berakhir</label>
                        <input type="date" class="form-control" id="tanggal_berakhir" name="tanggal_berakhir" value="<?= htmlspecialchars($data['tanggal_berakhir'] ?? '') ?>">
                    </div>
                </div>
                <div class="col-md-4">
                     <div class="mb-3">
                        ...
                        <select class="form-select" id="status_promo" name="status_promo">
                            <option value="draft" <?= ($data['status_promo'] ?? '') == 'draft' ? 'selected' : '' ?>>Draft</option>
                            <option value="aktif" <?= ($data['status_promo'] ?? '') == 'aktif' ? 'selected' : '' ?>>Aktif</option>
                            <option value="tidak_aktif" <?= ($data['status_promo'] ?? '') == 'tidak_aktif' ? 'selected' : '' ?>>Tidak Aktif</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="isi_pesan" name="isi_pesan" rows="3" placeholder="Tulis pesan yang akan dikirim ke pelanggan..."><?= htmlspecialchars($data['isi_pesan']) ?></textarea>
            </div>
        </div>
